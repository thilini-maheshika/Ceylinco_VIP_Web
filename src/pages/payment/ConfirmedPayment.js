    ...
    try {
      const [dealerResponse, paymentResponse] =
        await Promise.all([
          ...
          Axios.get(config.url + '/payment/all/confirm', { headers: { 'x-token': getToken() } }),
        ]);
  ...
  const handleUpdateStatus = async () => {
    try {
      ...
      const response = await Axios.put(
        config.url + '/payment/status/' + paymentid, data,
        {
          headers: {
            'Content-Type': 'application/json',
            'x-token': getToken()
    ...
    <>
      <div className="tabled">
        <Row gutter={[22, 0]}>
          <Col xs={24} xl={24}>
            <SimpleTable
              tableHeading="Customer Confirmed Payment"
              columns={columns}
              dataSet={paymentList}
              isLoading={isLoading}
              idName="paymentid"
              ...
              renderRowActionMenuItems={({ row, closeMenu }) => (
                [
                  <MenuItem key={1} onClick={() => {
                    handleOpenStatus(row.id)
                    closeMenu();
                  }} style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                    <PaidIcon /> Complete Payment
                  </MenuItem>,
        ...
        <DialogBox open={openstatus} desc="Are you sure you want to change to comeplete?" title="Complete Payment" buttonText="Confirm" handleClose={handleClose} handleDelete={handleUpdateStatus} />
      </div>
    </>
  )
}
